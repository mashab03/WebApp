...
if(isset($_GET['edit'])){
	...
	if($rs){
		echo "<script>alert('Data berhasil diubah!');window.location='?cat=dinas&page=wilayah'</script>";
	}else{
		echo "<script>alert('Data gagal diubah!');window.location='?cat=dinas&page=wilayah'</script>";
	}
}
?>
<form action="?cat=dinas&page=editwilayah&edit=1&id=<? echo $id;?>" method="post">       
