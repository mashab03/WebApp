...
if(isset($_GET['edit'])){
	...
	if($rs){
		echo "<script>alert('Data berhasil diubah!');window.location='?cat=dinas&page=penerima'</script>";
	}else{
		echo "<script>alert('Data gagal diubah!');window.location='?cat=dinas&page=penerima'</script>";
	}
}
?>
<form action="?cat=dinas&page=editpenerima&edit=1&id=<? echo "$id"?>" method="post"> 
